...
`PROCESS.md` carries three or four curated moments. That curation silently
discards everything not chosen — including the corrections, the dead ends and
the near-misses that are the most honest evidence of how the work went.
...
- **Always the absolute GitHub URL**, exactly as written above:
  `[`<7-char sha>`](https://github.com/comp4020-agentic-coding-studio/comp4020-ass1-jnheinrich451-eng/commit/<7-char sha>)`.
...
- **Cite the work commit, then commit the record separately.** The history
  already runs this way — `5e48816` made the change, `6414dfa` recorded it — and
  it keeps the cited hash pointing at the change being described rather than at
  the paragraph describing it. Practically: finish the work, commit it, read the
  hash back with `git log --format=%h -1`, write the entry, commit the entry.

## Promoting an entry to PROCESS.md

`PROCESS.md` takes three or four moments for an assignment. Choose them from
this file, and prefer:
...
Entries written before this skill existed carry their verification **inside**
`**Result:**` rather than under a `**Verified:**` heading. That is expected; do
not rewrite them to match. New entries use the fuller shape.
